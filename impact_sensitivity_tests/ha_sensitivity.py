...
for l in impact_ls:
    agent_dict = {}
    env = BankSimEnv(Cifuentes_Impact_lambda=l, shock=0.05)
    env.reset()
    ...
    for idx, name in enumerate(bank_names):
        ...
        agent = Heuristic_Agent(bar=bar, name=name)
    ...
    for episode in range(GAME_PARAMS.EPISODES):
        ...
        while play < GAME_PARAMS.MAX_PLAY:
            ...
            for bank_name, bank in env.allAgentBanks.items():
                ...
                # choose action
                actions[bank_name] = agent_dict[bank_name].act(current_obs[bank_name].astype(float), env=env)
...
ax.set(xlabel='initial asset shock', ylabel='end of episode equity',
       title=f'Heuristic Action buffer = {bar}, five agents')
# ax.grid()
plt.show()
